...
- (void)restartHand{
    ...
    _moneyLabel.text = [NSString stringWithFormat:(@"$%d"),_gameModel.money];
    if(_gameModel.money <= 10){
        (_gameModel.bet = 1);
    }else{
        _gameModel.bet = 10;
    }
     _betLabel.text = [NSString stringWithFormat:(@"Bet: %d"),_gameModel.bet];
}
...
- (IBAction)increaseButtonTapped:(id)sender {
    ...
    if(_gameModel.bet > _gameModel.money){
        _gameModel.bet = _gameModel.money;
    }
